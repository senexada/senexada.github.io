...
* RCT [Colonoscopy study][rct-colon] -- no all cause mortality boost --
  [local copy][rct-colon-local]
  > During the 10-year follow-up period, 3036 participants in the invited
  > group (11.03%) died from any cause, as compared with 6079 (11.04%) in
  > the usual-care group (risk ratio, 0.99; 95% CI, 0.96 to 1.04) 
  

[rct-colon]: https://www.nejm.org/doi/full/10.1056/NEJMoa2208375#t2
[rct-colon-local]: data-files/2025/06/rct-colon.pdf



### Misc:

* Harvard Biz School -- [Historic Strategies][old-overload] for dealing with info overload --
  [local copy][old-overload-local] --
  > In one of the most widely diffused manuals on note-taking, the Jesuit Jeremias Drexel explained:
  > “Reading is useless, vain and silly when no writing is involved, unless you are
  > reading [devotionally] Thomas a Kempis or some such. Although I would not
  > want even that kind of reading to be devoid of all note-taking.”
  > [...]
  > This process of “excerpting” was said to aid the memory in at least two ways: the act
  >  of writing itself helped to ingrain the passage in the memory, and the excerpts
  > could then be learned by heart to exercise the memory.
  > [...]
  > Sacchini, the superdiligent Jesuit, favored copying out passages twice, once
  > in a notebook kept in the order in which the passages were encountered in the
  > text, and a second time to sort them under appropriate topical headings
...
[old-overload]: https://dash.harvard.edu/server/api/core/bitstreams/7312037c-6086-6bd4-e053-0100007fdf3b/content
[old-overload-local]: data-files/2025/06/info-overload.pdf
...
* blog post on [Early Christian Textual Transmission][text-xmit]

* Info on [Thucydides manuscripts][thucy-info] -- 8 manuscripts, earliest is 1100 years after events, plus a few
  fragments within 700 years

* Greek [manuscript summary][greek-manuscripts] -- major manuscripts and dates for many greek authors --
  [local copy][greek-manuscripts-local]

* [blog post][8-thucy] on the 8 thucydides manuscripts, referencing an academic book



[text-xmit]: https://brentnongbri.com/2018/04/09/early-christian-textual-transmission-part-3/
[thucy-info]: https://thereformedmind.wordpress.com/2011/03/23/selective-manuscript-agnosticism/
[greek-manuscripts]: https://www.tertullian.org/rpearse/manuscripts/greek_classics.htm#Thucydides
[greek-manuscripts-local]: data-files/2025/06/greek-manuscripts.html
[8-thucy]: https://theglitteringeye.com/the-manuscript-tradition-of-thucydidess-the-history-of-the-pelopponesian-war/
...
* Transfer files onto DC-1
  ...
  * download from laptop by serving http on laptop
      > $ python3 -m http.server   __// maybe include --bind 0.0.0.0 __
      >
      > $ ifconfig | grep inet
      >
      > (then on DC-1, point browser to ip:port)
      > (be sure DC-1 is on same network as laptop -- guest vs regular)
